...
export class EditMachineDialogController {
  ...
  onNameChange(name: string): void {
    ...
    const environment = this.environmentManager.renameMachine(this.copyEnvironment, this.machine.name, name);
  ...
  isRecipeValid(): che.IValidation {
    try {
      ...
      if (!this.isAdd && this.cheRecipeService.isOpenshift(this.copyEnvironment.recipe)) {
  ...
  private parseMachineRecipe(): void {
    try {
      ...
      const environment = this.checkMachineNameChanges(this.copyEnvironment);
  ...
  private getFullName(name: string): string {
    ...
    return this.machine.name.replace(new RegExp(`${oldName}$`), name);
  }

  /**
   * Checks machine name changes.
   * @param {che.IWorkspaceEnvironment} environment
   * @returns {che.IWorkspaceEnvironment}
   */
  private checkMachineNameChanges(environment: che.IWorkspaceEnvironment): che.IWorkspaceEnvironment {
    // check recipe machine name
    ...
    if (this.machineName === newMachineName) {
      ...
      return this.environmentManager.renameMachine(environment, this.machine.name, newMachineName);
    }
  }
  ...
  private checkCriticalRecipeChanges(): void {
    if (this.isAdd) {
      this.copyEnvironment = angular.copy(this.environment);
      this.copyEnvironment = this.environmentManager.addMachine(this.copyEnvironment, this.machine);
    } else if (this.cheRecipeService.isOpenshift(this.copyEnvironment.recipe)) {
